...
bool segmentIntersection(Point a1, Point a2, Point b1, Point b2) {
    if (dcmp(Cross(a2 - a1, b2 - b1)) == 0) return false;
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (segmentIntersection(a, b, { 0, 0 }, Point(double(w) / u * v, w))) {
            vec.pb(getLineIntersection(a, b - a, { 0, 0 }, Vector(double(w) / u * v, w)));
        }
    }
    sort(all(vec));
    uni(vec);
    if (vec.size() < 2) { cout << double(w) / u << '\n'; return 0; }
    ...
    while (!pq.empty()) {
        ...
        if (cur.t <= t) continue;
        double difft = cur.t - t;
        double difft2 = double(cur.y - pos) / u;
