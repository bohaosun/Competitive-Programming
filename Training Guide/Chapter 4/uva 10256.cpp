...
int na, nb, ha, hb;
Point tmpa[N], tmpb[N], hulla[N], hullb[N];
Point *a, *b;
...
int main() {
    ...
    while (cin >> na >> nb && na && nb) {
        for (int i = 0; i < na; i++) cin >> tmpa[i].x >> tmpa[i].y;
        for (int i = 0; i < nb; i++) cin >> tmpb[i].x >> tmpb[i].y;
        ...
        if (na == 1) {
            if (nb == 1){
                ...
                if (!onSegment(a[0], b[0], b[1])) cout << "Yes\n";
                else cout << "No\n";
            } else {
                hb = convexHull(b, nb, hullb);
                if (!isPointInPolygon(a[0], hullb, hb)) cout << "Yes\n";
                else cout << "No\n";
            }
        } else if (na == 2) {
            if (nb == 2) {
                if (!segmentIntersection(a[0], a[1], b[0], b[1])) cout << "Yes\n";
                else cout << "No\n";
            } else {
                hb = convexHull(b, nb, hullb);
                bool ok = true;
                for (int i = 0; i < hb; i++) {
                    if (segmentIntersection(a[0], a[1], hullb[i], hullb[(i + 1) % hb])) { ok = false; break; }
                }
                if (ok) cout << "Yes\n";
                else cout << "No\n";
            }
        } else {
            ha = convexHull(a, na, hulla);
            hb = convexHull(b, nb, hullb);

            bool ok = true;
            for (int i = 0; i < ha; i++) {
                Point a1 = hulla[i], a2 = hulla[(i + 1) % ha];
                for (int j = 0; j < hb; j++) {
                    Point b1 = hullb[j], b2 = hullb[(j + 1) % hb];

                    if (segmentIntersection(a1, a2, b1, b2)) { ok = false; break; }
                }
                if (!ok) break;
            }
            if (!ok) { cout << "No\n"; continue; }

            if (isPointInPolygon(hulla[0], hullb, hb) || isPointInPolygon(hullb[0], hulla, ha)) cout << "No\n";
            else cout << "Yes\n";
        }
    }
    return 0;
