...
double polygonArea(Point *p, int n) {  // size >= 3
    double area = 0;
    assert(n >= 3);
...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = -1; j <= 1; j += 2) {
                for (int k = -1; k <= 1; k += 2) {
                    Point cur{ x + w / 2 * j, y + h / 2 * k };
                    cur = c + Rotate(cur - c, theta * PI / 180);
                    p[tot++] = cur;
                }
            }
            area += w * h;
        ...
        tot = convexHull(p, tot, ans);

        cout << area / polygonArea(ans, tot) * 100 << " %\n";
    }
    return 0;
}
