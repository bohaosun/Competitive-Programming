...
Vector Normal(Vector A) {  // Norm with unit length, A != 0
    double L = Length(A);
...
int main() {
    ...
    while (cin >> x1 >> y1 >> r1 >> x2 >> y2 >> r2) {
        ...
        sort(all(ans));
        for (int i = 0; i < cnt; i++) {
            cout << ans[i].p1.x << ' ' << ans[i].p1.y << ' ' << ans[i].p2.x << ' ' << ans[i].p2.y;
            cout << ' ' << Length(ans[i].p1 - ans[i].p2) << '\n';
        }
    }
    return 0;
}
