...
double solve(Point p1, Point p2, double sum_x, double sum_y) {
    double A, B, C;
    Vector v = p2 - p1;
    if (dcmp(v.x)) {
        A = v.y / v.x;
    ...
    return abs(A * sum_x + B * sum_y + n * C) / sqrt(sqr(A) + sqr(B));
}
...
int main() {
    ...
    for (int kase = 1; kase <= T; kase++) {
        ...
        hull_sz = convexHull(p, n, hull);

        //for (int i = 0; i < hull_sz; i++) cout << hull[i].x << ' ' << hull[i].y << endl;
        ...
        for (int i = 0; i < hull_sz; i++) ans = min(ans, solve(hull[i], hull[(i + 1) % hull_sz], sum_x, sum_y));
        //cout << solve(hull[0], hull[1], sum_x, sum_y) << endl;
