    ...
    while (cin >> n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                int cnt = 0;
                for (int k = 0; k < 3; k++) {
                    if (dcmp(v[j][k] - v[i][k]) >= 0) cnt++;
                }
                if (cnt == 3) { ok = false; break; }
                //if (cnt == 0) continue;

                double A = K / v[j][0] - K / v[i][0] - K / v[j][2] + K / v[i][2];
                double B = K / v[j][1] - K / v[i][1] - K / v[j][2] + K / v[i][2];
                double C = K / v[j][2] - K / v[i][2];
            ...
            Point tmp[N];
            if (halfPlaneIntersection(L, m, tmp)) cout << "Yes\n";
            else cout << "No\n";
        }
    }
    return 0;
