...
bool check(Point cur) {
    for (auto &it : lines) {
        if (onSegment(cur, it._1, it._2)) return false;
    }
    return true;
}
...
int main() {
    ...
    while (cin >> n && n) {
        ...
        p[tot++] = { 500, 500 };
        for (int i = 0; i < n; i++) {
            Point cur = lines[i]._1;
            if (check(cur)) p[tot++] = cur;   // Must Check!!!
            cur = lines[i]._2;
            if (check(cur)) p[tot++] = cur;   // Must Check!!!
        }
        ...
        for (int i = 0; i < tot; i++) {
            for (int j = 0; j < tot; j++) {
                bool ok = true;
                for (int k = 0; k < n; k++) {
                    if (segmentProperIntersection(p[i], p[j], lines[k]._1, lines[k]._2)) { ok = false; break; }
                }
