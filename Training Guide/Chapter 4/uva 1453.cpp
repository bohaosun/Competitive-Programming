...
const int N = 112345 * 4;

Point p[N], hull[N];
int n;

double d2(Point *p, int sz) {
    double ans = 0;

    int pos = 0;
    for (int i = 0; i < sz; i++) {
        while (Length2(p[(pos + 1) % sz] - p[i]) >= Length2(p[pos] - p[i])) pos = (pos + 1) % sz;
        ans = max(ans, Length2(p[pos] - p[i]));
    }
    return ans;
}
...
int main() {
    ...
    while (T--) {
        ...
        int sz = convexHull(p, cnt, hull);

        //cout << int(d2(hull, sz) + 0.5) << '\n';
        cout << int(diameter(hull, sz)) << '\n';
    }
    return 0;
}
